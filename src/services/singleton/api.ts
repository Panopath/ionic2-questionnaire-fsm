...
export class Api {
  ...
  login(username: string, password: string): Promise<loginResponse> {
    return this.request("user.login", {username, password}, {muteErrorToast: true});
  ...
  @RequiresLogin
  queryWorker(id_type, id_no): Promise<queryWorkerResponse> {
    return this.request("worker.query", {id_type, id_no}, {});
  }
  ...
  @RequiresLogin
  uploadReport(location_id: number, images: Array<any>, problems: Array<any>, details: string): Promise<null> {
    return this.request("report.upload", {location_id, images, problems, details}, {});
  }

  @RequiresLogin
  getMonthlyHistory(showLoading = false): Promise<getMonthlyHistoryResponse> {
    return this.request("user.monthly", {}, {showLoading}).then((data: Array<any>) => {
      return new Promise((resolve) => {
        data.forEach((value) => {
          let problems, images;
          try {
            problems = JSON.parse(value["problems"]);
          } catch (ex) {
            problems = [];
          }
  ...
  @CachesResult
  getLocations(showLoading = false): Promise<getLocationsResponse> {
    return this.request("location.get", {}, {showLoading});
  }
}
